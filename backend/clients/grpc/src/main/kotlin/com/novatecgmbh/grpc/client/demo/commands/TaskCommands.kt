...
class TaskCommands(private val callCredentials: CallCredentials) {
    ...
    @GrpcClient("grpcapi")
    private lateinit var taskServiceBlocking: TaskServiceGrpc.TaskServiceBlockingStub

    @GrpcClient("grpcapi") private lateinit var taskService: TaskServiceGrpc.TaskServiceStub

    private val subscriptions:
        MutableMap<ProjectIdentifier, ClientCallStreamObserver<ProjectIdentifier>> =
        mutableMapOf()
    ...
    fun subscribe(identifier: String) {
        ProjectIdentifier.newBuilder().setValue(identifier).build().let {
            val observer: StreamObserver<Task> =
                object : StreamObserver<Task>, ClientResponseObserver<ProjectIdentifier, Task> {
                    ...
                    override fun beforeStart(
                        requestStream: ClientCallStreamObserver<ProjectIdentifier>
                    ) {
                        subscriptions[ProjectIdentifier.newBuilder().setValue(identifier).build()] =
                            requestStream
                    }
                }
