...
class SchemaExporter {
    ...
    private fun generateIndicesForForeignKeys(metadata: Metadata, outputPath: String) {
            ...
            namespace.tables.forEach { table: Table ->
                table.foreignKeys.values.forEach foreignKeysExitLabel@{ foreignKey: ForeignKey ->
                    ...
                    val uniqueKeyColumns =
                        iterable.map { it.columns }.filter { it.size == 1 }.flatten()
        ...
        if (statements.isNotEmpty()) {
            // If statements are not empty then add a new line to separate index statements from
            // previous
            // statements
            statements.add(0, "\n")
    ...
    /**
     * Code is a mix of [org.hibernate.tool.schema.internal.StandardIndexExporter] and adjustments
     * to make it work without rewriting original hibernate schema exporter.
     *
