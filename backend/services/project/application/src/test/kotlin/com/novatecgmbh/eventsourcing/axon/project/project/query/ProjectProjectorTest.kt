...
class ProjectProjectorTest {
    ...
    fun setUp() {
        ...
        justRun {
            updateEmitter.emit(
                MyProjectsQuery::class.java,
                any(),
                capture(projectQueryResultsCaptor)
            )
        }
    }
    ...
    private fun `verify that save and emit are called with correct arguments`(
        ...
        if (expectedCompanyReference != null)
            assertEquals(
                expectedCompanyReference,
                projectProjectionCaptor.captured.companyReference
            )
        ...
        for (projectQueryResult in projectQueryResultsCaptor) {
            ...
            if (expectedStartDate != null)
                assertEquals(expectedStartDate, projectQueryResult.startDate)
            if (expectedDeadline != null)
                assertEquals(expectedDeadline, projectQueryResult.deadline)
