...
class ProjectAuthorizer(
    ...
    fun register() {
        commandBus.registerHandlerInterceptor(
            this as MessageHandlerInterceptor<in CommandMessage<*>>
        )
    }
        ...
        if (payload is ProjectCommand) {
            when (payload) {
                ...
                is UpdateActualScheduleInternalCommand -> {}
                    ...
                    throw IllegalStateException("Authorization rule missing for project command")
            }
        }
    ...
    fun authorize(command: CreateProjectCommand, userId: UserId) {
        if (
            !projectAclRepository.hasPermissionToCreateProjectForCompany(
                userId,
                command.companyId.identifier
            )
        ) {
    ...
    fun authorize(command: UpdateProjectCommand, userId: UserId) {
        if (
            !projectAclRepository.hasAccessToProject(userId, command.aggregateIdentifier.toString())
        ) {
