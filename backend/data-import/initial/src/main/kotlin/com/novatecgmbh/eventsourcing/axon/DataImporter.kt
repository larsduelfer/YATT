...
class DataImporter(private val commandGateway: CommandGateway) : CommandLineRunner {
    ...
    private fun createCompaniesAndEmployees() {
        ...
        commandGateway
            .send<Unit>(CreateCompanyCommand(companyIds[0], "Novatec Consulting GmbH"))
            .join()
        ...
        commandGateway
            ...
            .join()
        commandGateway
            .send<Unit>(CreateEmployeeCommand(EmployeeId(), companyIds[0], userIds[1]))
            .join()
        commandGateway
            .send<Unit>(CreateEmployeeCommand(employeeIdOfUser1, companyIds[1], userIds[1]))
            .join()
        commandGateway
            .send<Unit>(CreateEmployeeCommand(EmployeeId(), companyIds[1], userIds[2]))
            .join()
        commandGateway
            .send<Unit>(CreateEmployeeCommand(employeeIdOfUser2, companyIds[2], userIds[2]))
            .join()
        commandGateway
            .send<Unit>(CreateEmployeeCommand(EmployeeId(), companyIds[2], userIds[0]))
            .join()
